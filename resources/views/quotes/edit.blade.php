        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-brand-lt py-3">
                <h3 class="card-title text-brand fw-bold mb-0">
                    <i class="ti ti-file-text me-2"></i> Editar Cotización #{{ str_pad($quote->id, 5, '0', STR_PAD_LEFT) }}
                </h3>
            </div>
            <div class="card-body">
                <form id="form-quote">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="date" name="date" class="form-control" value="{{ date('Y-m-d', strtotime($quote->date)) }}" required>
                        </div>
                        <div class="col-md-8">
                            <label class="form-label fw-bold">Señor(es) / Razón Social</label>
                            <input type="text" name="client_name" class="form-control" placeholder="Ej. Empresa SAC" value="{{ $quote->client_name }}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold">RUC / DNI</label>
                            <div class="input-group">
                                <input type="text" name="client_ruc" class="form-control" placeholder="Ej. 20123456789" value="{{ $quote->client_ruc }}" required>
                        ...
                        <div class="col-md-8">
                            ...
                            <input type="text" name="client_address" class="form-control" placeholder="Ej. Av. Principal 123" value="{{ $quote->client_address }}" required>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive mb-4">
                        <table class="table table-vcenter">
                            ...
                            <tbody id="products-container">
                                @php
                                    $quoteProducts = is_string($quote->products) ? json_decode($quote->products, true) : $quote->products;
                                @endphp
                                @if(is_array($quoteProducts) && count($quoteProducts) > 0)
                                    @foreach($quoteProducts as $index => $qp)
                                    <tr>
                                        <td>
                                            <select name="products[{{ $index }}][id]" class="form-select product-select" required>
                                                <option value="">Seleccione un producto</option>
                                                @foreach($products as $p)
                                                    <option value="{{ $p->id }}" data-price="{{ $p->calculated_price }}" {{ $p->id == $qp['id'] ? 'selected' : '' }}>{{ $p->name }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                                            <input type="number" min="1" step="1" name="products[{{ $index }}][quantity]" class="form-control" value="{{ isset($qp['quantity']) ? $qp['quantity'] : 1 }}" required>
                                        </td>
                                        <td>
                                            <input type="number" step="0.01" name="products[{{ $index }}][price]" class="form-control product-price" value="{{ $qp['price'] }}" required>
                                        </td>
                                        <td>
...
@section('scripts')
<script>
    let rowIdx = {{ is_array($quoteProducts = is_string($quote->products) ? json_decode($quote->products, true) : $quote->products) ? count($quoteProducts) : 1 }};
