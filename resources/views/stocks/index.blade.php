	...
	<div class="table-responsive">
		<table class="table card-table table-vcenter">
			...
			<tbody>
				...
				<tr>
					<td>{{ $stock->name }}</td>
					<td>{{ $stock->literage ?? '-' }}</td>
					<td>{{ $stock->location ?? '-' }}</td>
					<td>
                        @if($stock->status == 'Activo' || strtolower($stock->status) == 'activo')
						<span class="badge bg-success text-success-fg">{{ $stock->status }}</span>
						@else
						<span class="badge bg-secondary text-secondary-fg">{{ $stock->status }}</span>
						@endif
                    </td>
					<td>{{ $stock->stock }}</td>
					<td>
						<div class="d-flex gap-2">
							<button class="btn btn-icon btn-edit-corporate btn-edit" data-id="{{ $stock->id }}" data-bs-toggle="tooltip" title="Editar">
								<i class="ti ti-pencil icon"></i>
							</button>
							<button class="btn btn-icon btn-delete-corporate btn-delete" data-id="{{ $stock->id }}" data-bs-toggle="tooltip" title="Eliminar">
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content shadow-lg border-0">
			<form id="storeForm" method="POST">
                ...
                <div class="px-3">
                    <p class="text-muted small mb-0 px-1">Completa los datos para registrar un nuevo stock.</p>
                </div>
				<div class="modal-body pt-3">
					<div class="row">
						...
						<div class="col-lg-6">
							<div class="mb-3">
								<label class="form-label fw-bold">Litraje</label>
								<div class="input-icon">
                                    <span class="input-icon-addon">
                                        <i class="ti ti-droplet text-muted"></i>
                                    </span>
                                    <input type="text" class="form-control" name="literage">
                                </div>
							</div>
						</div>
						<div class="col-lg-6">
				...
				<div class="modal-footer border-0">
					...
					<button type="submit" class="btn btn-brand px-4 shadow-sm fw-bold">
                        <i class="ti ti-device-floppy icon me-1"></i> Guardar
                    </button>
				</div>
			</form>
		</div>
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content shadow-lg border-0">
			<form id="editForm" method="POST">
                ...
                <div class="px-3">
                    <p class="text-muted small mb-0 px-1">Actualiza la información del stock seleccionado.</p>
                </div>
				<div class="modal-body pt-3">
					<div class="row">
						<div class="col-lg-12">
						...
						<div class="col-lg-6">
							<div class="mb-3">
								...
								<div class="input-icon">
                                    ...
                                    <input type="text" class="form-control" name="literage" id="editLiterage">
                                </div>
							</div>
						</div>
						<div class="col-lg-6">
				...
				<div class="modal-footer border-0">
					...
					<button type="submit" class="btn btn-brand px-4 shadow-sm fw-bold">
                        <i class="ti ti-device-floppy icon me-1"></i> Actualizar
                    </button>
				</div>
			</form>
		</div>
...
<script>
	...
	$(document).on('click', '.btn-edit', function(){
		...
		$.ajax({
			url: '{{ route('stocks.index') }}' + '/' + id + '/edit/',
			method: 'GET',
			success: function(data){
				$('#editName').val(data.name);
				$('#editLiterage').val(data.literage);
