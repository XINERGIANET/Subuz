			...
			<form class="mb-3">
						...
						<div class="col-lg-2">
							<div class="mb-3">
								...
								<select class="form-select" name="type">
									...
									<option value="Pago pendiente" {{ request()->type == 'Pago pendiente' ? 'selected' : '' }}>
						...
						<div class="col-lg-2">
							<div class="mb-3">
								...
								<select class="form-select" name="payment_method_id">
									<option value="">Seleccionar</option>
									<option value="credit" {{ request()->payment_method_id == 'credit' ? 'selected' : '' }}>
										Crédito</option>
							...
							<div class="mb-3">
								...
								<select class="form-select" name="delivery_status">
									...
									<option value="delivered" {{ request()->delivery_status == 'delivered' ? 'selected' : '' }}>
										Entregado</option>
									<option value="pending" {{ request()->delivery_status == 'pending' ? 'selected' : '' }}>No
										entregado</option>
								</select>
							</div>
						</div>
				...
				<div class="mt-3 d-flex justify-content-between align-items-center">
					<div>
						...
						<a href="{{ route('sales.index') }}" class="btn btn-outline-secondary ms-2"><i
								class="ti ti-rotate-clockwise icon"></i> Limpiar filtros</a>
					</div>
					@if(isset($total_by_type) && $total_by_type !== null)
						<div class="text-end d-flex gap-2 flex-wrap justify-content-end">
							<div class="p-2 px-3 border rounded-2 bg-azure-lt shadow-sm">
								<span class="text-azure small text-uppercase fw-bold">Total {{ request('type') }}
									Entregado:</span>
								<span
									class="fs-2 fw-bold text-azure ms-2">S/{{ number_format($total_by_type_delivered, 2) }}</span>
							</div>
							<div class="p-2 px-3 border rounded-2 bg-warning-lt shadow-sm">
								<span class="text-warning small text-uppercase fw-bold">Total {{ request('type') }} No
									Entregado:</span>
								<span
									class="fs-2 fw-bold text-warning ms-2">S/{{ number_format($total_by_type_not_delivered, 2) }}</span>
							</div>
						</div>
					@endif
				</div>
							...
							<tr>
								...
								<td>
									@if($sale->payments->count() > 0)
										...
										return optional($payment->payment_method)->name; })->filter()->unique()->implode(', ') }}
									@else
								...
								<td>
									<div class="d-flex gap-2">
										<button class="btn btn-icon btn-show" data-id="{{ $sale->id }}" data-bs-toggle="tooltip"
											title="Detalle venta">
										...
										@endif
										@if((auth()->user()->hasRole('despachador') || auth()->user()->hasRole('admin') || auth()->user()->hasRole('asistente')) && $isDelivered)
											<button class="btn btn-icon btn-reopen-delivery" data-id="{{ $sale->id }}"
												data-bs-toggle="tooltip" title="Volver a no entregado">
					...
					<div class="modal-body">
						...
						<div class="mb-3">
							...
							<div class="p-2 bg-light rounded border mb-3">
								<div class="row g-2">
									<div class="col-7">
										<select class="form-select form-select-sm ts-products-dispatch"
											id="add-dispatch-product-id">
									...
									<div class="col-3">
										<input type="number" class="form-control form-control-sm" id="add-dispatch-quantity"
											value="1" min="1">
									</div>
									<div class="col-2">
										<button type="button" class="btn btn-sm btn-brand w-100"
											id="btn-add-dispatch-detail">
							...
							<div class="row g-2">
								<div class="col-md-12 mb-2">
									...
									<input type="text" name="guide" id="dispatch_guide_input" class="form-control"
										placeholder="Ej: GR-00001" required>
								</div>
								<div class="col-md-12">
									<label class="form-label small text-muted mb-1">Foto de Evidencia</label>
									<input type="file" name="photo" id="dispatch_photo" class="form-control"
										accept="image/*" capture="camera" required>
									<div id="photo_preview_container" class="mt-2 text-center d-none">
										<img id="photo_preview" src="#" alt="Vista previa"
						...
						<div id="payment-question-container">
							<div class="mb-3">
								...
								<div class="btn-group w-100" role="group">
									...
									<label
										class="btn btn-outline-success py-2 d-flex align-items-center justify-content-center gap-2"
										for="dispatch_paid_yes">
										<i class="ti ti-check fs-2"></i> Si, pagado
									</label>
									<input type="radio" class="btn-check" name="paid" id="dispatch_paid_no" value="0">
									<label
										class="btn btn-outline-danger py-2 d-flex align-items-center justify-content-center gap-2"
										for="dispatch_paid_no">
				...
				<div class="modal-body bg-light-lt">
					...
					<div id="report-content">
						...
						<div class="card border-0 shadow-sm p-3 mb-3">
							<h4 class="fw-bold text-danger border-bottom pb-2 mb-3">Ingresos por Método (Efectivo, Yape,
								etc)</h4>
